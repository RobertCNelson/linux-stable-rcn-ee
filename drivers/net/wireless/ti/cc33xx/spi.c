...
{
	WARN_ON(glue->locking_pid != current->pid);
	WARN_ON(!glue->lock_count);
...
static int spi_cc33xx_probe(struct spi_device *spi)
{
	...
	ret = wlcore_probe_of(spi, glue, &gpio_irq, pdev_data);
	if (ret) {
		dev_err(glue->dev,
	...
	ret = request_threaded_irq(gpio_irq, cc33xx_spi_irq_hard_handler,
				   cc33xx_spi_irq_handler, irq_flags,
				   pdev_data->family->name, spi);
	if (ret) {
...
MODULE_DEVICE_TABLE(spi, cc33xx_spi_ids);
