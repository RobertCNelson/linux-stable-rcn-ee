 ...
 *
 * derived from the imx-rpmsg-tty implementation.
...
/*
 * struct rpmsgtty_port - Wrapper struct for rpmsg tty port.
 * @port:		TTY port data
 */
...
static int rpmsg_tty_probe(struct rpmsg_device *rpdev)
{
	...
	if (ret < 0) {
		dev_err(&rpdev->dev, "failed to register rpmsg tty: ret %d\n", ret);
		goto error1;
	} else {
