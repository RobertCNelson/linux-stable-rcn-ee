...
static int omap_gpio_probe(struct platform_device *pdev)
{
	...
	irqc->irq_ack = gpio_mask_ack_irq,
...
static void omap2plus_enable_irqdesc(struct ipipe_domain *ipd, unsigned irq, struct irq_chip *irqc)
{
	struct irq_desc *desc = irq_to_desc(irq);
	struct irq_data *idata = irq_desc_get_irq_data(desc);
...
static void omap2plus_disable_irqdesc(struct ipipe_domain *ipd, unsigned irq, struct irq_chip *irqc)
{
	struct irq_desc *desc = irq_to_desc(irq);
	struct irq_data *idata = irq_desc_get_irq_data(desc);
	struct irq_chip *chip = irq_data_get_irq_chip(idata);
